...
class Motors(object):
    ...
    def __init__(self, a_star, odometer):
        ...
        self.max_speed = float(1440 * 150 / 60.)
    ...
    def set_speed_target(self, left, right):
        ...
        self.speed_command = SpeedCommand(self.convert_command_to_speed(left),
    ...
    def _odom_measurement_callback(self, speed_left, speed_right, x, y, yaw, omega, dist,
        ...
        if self.distance_command is not None:
            set_point_distance = self.distance_command.distance \
                                 + self.distance_command.initial_distance
            set_point_yaw = self.distance_command.yaw
            max_speed = self.distance_command.speed
            if abs(dist - set_point_distance) < 0.01 \
                    or abs(self.distance_command.initial_distance - dist) - \
                    abs(self.distance_command.distance) > 0.00 \
                    or self.command_reached:
                self.odometer.stop_tracking()
                self.command_reached = True
                speed_cmd = 0
            else:
                speed_cmd = self.pid_distance.get_output(set_point_distance * 1000, dist * 1000,
                                                         current_time)
            ...
            # print("yaw: {} set_point_yaw:{}".format(convert_to_deg(yaw),
            # convert_to_deg(set_point_yaw)))
            ...
            if abs(convert_to_deg(yaw - set_point_yaw)) < 0.5 or self.command_reached:
                ...
                rotation_speed = self._get_yaw_delta(set_point_yaw, yaw, self.pid_rotation_angle,
                                                     current_time)
            ...
            # print("yaw: {} set_yaw:{} omega:{}".format(convert_to_deg(yaw),
            # convert_to_deg(set_point_yaw),
            # convert_to_deg(omega)))
        ...
        #    print("left_cmd: {} right_cmd: {}".format(int(left_cmd/self.max_cmd*100),
        #    int(right_cmd/self.max_cmd*100)))
